...
module.exports = (input) => {
  ...
  function parseCategories() {
    try {
      var categoryFile = yaml.safeLoad(fs.readFileSync(path + "/categories.yml").toString());
  ...
  async function updateGlobals() {
    ...
    // ## Category lookup ## //
    DatabaseInfo.categoryList = parseCategories();
    ...
    try {
      ...
      for(i in DatabaseInfo.categoryList) {
        ...
        if(typeof questionPool.questions === "undefined") {
          throw new Error(`Unable to read questions from the category ${DatabaseInfo.categoryList[i].name}. (questions_${DatabaseInfo.categoryList[i].id}.yml) Ensure that the file is formatted correctly.`);
        }
      ...
      try {
        ...
        if(typeof token !== "undefined") {
          if(isRandomCategory) {
            ...
            if(config["debug-log"]) {
              console.log("Pushing " + question.id + " to token array (rand).");
            }
          }
          else {
            ...
            if(config["debug-log"]) {
              console.log("Pushing " + question.id + " to token array (" + category + ").");
            }
          }
        }
      }
  ...
  .catch((err) => {
    if(err) {
      console.error("Error occurred while attempting to load the file database: ");
